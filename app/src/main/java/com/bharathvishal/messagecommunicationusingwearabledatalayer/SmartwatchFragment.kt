...
import com.bharathvishal.messagecommunicationusingwearabledatalayer.databinding.FragmentSmartwatchBinding
 ...
 */
class SmartwatchFragment : Fragment(), CoroutineScope by MainScope(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentSmartwatchBinding.bind(view)
        ...
        binding.checkwearablesButton.setOnClickListener {
            if (!wearableDeviceConnected) {
                initialiseDevicePairing(requireContext())
            }
        }
    ...
    private fun initialiseDevicePairing(context: Context) {
        // Coroutine
        launch(Dispatchers.Default) {
            var getNodesResBool: BooleanArray? = null

            try {
                getNodesResBool = getNodes(context)
            } catch (e: Exception) {
                e.printStackTrace()
            }
            ...
            withContext(Dispatchers.Main) {
                if (getNodesResBool!![0]) {
                    ...
                    if (getNodesResBool[1]) {
                        Toast.makeText(
                            context,
                            "Smartwatch conectado éxitosamente.",
                            Toast.LENGTH_LONG
                        ).show()
                        ...
                        Toast.makeText(
                            context,
                            "Un Smartwatch está emparejado pero la aplicación wearable de tu reloj no está abierta. Abre la aplicación wearable e inténtalo de nuevo.",
                            Toast.LENGTH_LONG
                        ).show()
                        binding.deviceconnectionStatusTv.text =
                            "Un Smartwatch está emparejado pero la aplicación wearable de tu reloj no está abierta."
                        binding.deviceconnectionStatusTv.visibility = View.VISIBLE
                        wearableDeviceConnected = false
                    }
                } else {
                    Toast.makeText(
                        context,
                        "No hay ningún smartwatch emparejado. Vincula un smartwatch a tu teléfono e inténtalo de nuevo.",
                        Toast.LENGTH_LONG
                    ).show()
                    binding.deviceconnectionStatusTv.text =
                        "No hay ningún smartwatch emparejado."
                    binding.deviceconnectionStatusTv.visibility = View.VISIBLE
                    wearableDeviceConnected = false
                }
            }
